...
function load_admins(){
    ...
    $result = mysqli_query($mdb, "SELECT * from ".$prefix."_admin ORDER BY adminid ASC");
...
function do_add_admins(){
    ...
    $sql =  mysqli_query($mdb, "INSERT INTO ".$prefix."_admin (fullname,admin_name,password,email,regdate) VALUES ('$fullname','$admin_name','$password','$email',NOW())") or die ("Error Adding Mod: ". mysqli_error($mdb));
...
function edit_admins() {
    ...
    $result = mysqli_query($mdb, "SELECT * from ".$prefix."_admin WHERE adminid='".$_GET['adminid']."'");
...
function do_edit_admins(){
    global  $mdb,$prefix,$fullname,$admin_name,$password,$email,$adminid;

    if ($password == ""){
        $sql =  mysqli_query($mdb, "UPDATE  ".$prefix."_admin SET  fullname='$fullname',admin_name='$admin_name',email='$email' where adminid='$adminid'") or die ("Error Editing admins: ". mysqli_error($mdb));
    }else{
        $password = md5($password);
        $sql =  mysqli_query($mdb, "UPDATE  ".$prefix."_admin SET  fullname='$fullname',admin_name='$admin_name',password='$password',email='$email' where adminid='$adminid'") or die ("Error Editing admins: ". mysqli_error($mdb));
    }
...
function del_admins(){
    global  $mdb,$prefix,$adminid;

    if ($adminid == 1){
        die("You Cannot delete the Main Admin");
    }else{
        $sql =  mysqli_query($mdb, "delete from  ".$prefix."_admin where adminid=".$_GET["delid"]."") or die ("Error del admins: ". mysqli_error($mdb));
    }
