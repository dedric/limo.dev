...
function load_results(){
	global $mdb;
  $row_count = "0";
  $LSQL = mysqli_query($mdb, 'SELECT * FROM site_module_results ORDER by u_ymd DESC') or die("Invalid Result Query: " . mysqli_error($mdb));
...
function do_add_results(){
	global $mdb;
	@extract($_POST);
	$SQL = mysqli_query($mdb,'INSERT INTO site_module_results (name,o_ymd,u_ymd,b_pic,a_pic,ctext) VALUES
														("'.$name.'",NOW(),NOW(),"'.$before.'","'.$after.'","'.$ctext.'")') or die("Invalid Add Result Query: " . mysqli_error($mdb));
...
function do_del_results(){
	global $mdb;
	@extract($_GET);
	$SQL = mysqli_query($mdb,'DELETE FROM site_module_results WHERE id="'.$id.'" LIMIT 1') or die("Invalid Delete Result Query: " . mysqli_error($mdb));
...
function do_edit_results(){
	global $mdb;
	@extract($_POST);
	$SQL = mysqli_query($mdb, 'UPDATE site_module_results SET name="'.$name.'", ctext="'.$ctext.'", b_pic="'.$before.'", a_pic="'.$after.'", u_ymd=NOW() WHERE id="'.$id.'" LIMIT 1') or die('Invalid Update Result Query: ' . mysqli_error($mdb));
...
function edit_results(){
	global $mdb;
	@extract($_GET);
	$SQL = mysqli_query($mdb,'SELECT * FROM site_module_results WHERE id="'.$id.'" LIMIT 1') or die('Invalid Edit Result Query: ' . mysqli_error($mdb));
